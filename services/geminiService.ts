...
export const createChatSession = (apiKey?: string) => {
  ...
  // Use provided API key first, then fall back to env/localStorage
  const finalApiKey = apiKey || envKey || localStorageKey;
  ...
  console.log(
    "Creating chat session with API key:",
    finalApiKey.substring(0, 10) + "..."
  );
